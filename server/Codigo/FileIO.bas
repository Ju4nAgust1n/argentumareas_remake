    ...
    For Y = YMinMapSize To YMaxMapSize
        For X = XMinMapSize To XMaxMapSize
            ...
            If ByFlags And 4 Then
                ...
                Call addObjArea(MapData(map, X, Y).ObjInfo.objIndex, _
                                MapData(map, X, Y).ObjInfo.amount, _
                                map, X, Y)
...
'Agus; multiplicador de experiencia desde server.ini
RATE_EXP = GetVar(IniPath & "Server.ini", "INIT", "EXP")
...
Npclist(NpcIndex).GiveEXP = val(GetVar(npcfile, "NPC" & NpcNumber, "GiveEXP")) * RATE_EXP
