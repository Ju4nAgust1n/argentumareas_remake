...
Public RATE_EXP     As Single 'Agus; multiplicador de experiencia desde server.ini
...
On Error GoTo Errhandler
    ...
    With Npclist(NpcIndex)
        ...
        Call removeNpcArea(NpcIndex, Npclist(NpcIndex).Pos.map, Npclist(NpcIndex).Pos.X, Npclist(NpcIndex).Pos.Y)
...
Dim CharIndex As Integer
    ...
    Else
        ...
        Call addNpcArea(NpcIndex, Npclist(NpcIndex).Pos.map, Npclist(NpcIndex).Pos.X, Npclist(NpcIndex).Pos.Y)
    End If
End Sub
        ...
        If LegalPosNPC(.Pos.map, nPos.X, nPos.Y, .flags.AguaValida = 1, .MaestroUser <> 0) Then
            ...
            If UserIndex > 0 Then
                ...
                With UserList(UserIndex)
                    ...
                    Call new_SendToAreaByPos(PrepareMessageCharacterMove(UserList(UserIndex).Char.CharIndex, .Pos.X, .Pos.Y), .Pos.map, .Pos.X, .Pos.Y)
            ...
            'Call new_SendToAreaAndAdyacentByPos(PrepareMessageCharacterMove(.Char.CharIndex, nPos.X, nPos.Y), .Pos.map, nPos.X, nPos.Y)
            
            Call new_SendToNPCAreaByPos(PrepareMessageCharacterMove(Npclist(NpcIndex).Char.CharIndex, nPos.X, nPos.Y), Npclist(NpcIndex).Pos.map, nPos.X, nPos.Y, True)
...
'###################################################
    ...
    With Npclist(NpcIndex)
        ...
        .GiveEXP = val(Leer.GetValue("NPC" & NpcNumber, "GiveEXP")) * RATE_EXP
